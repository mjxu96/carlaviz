...
bool FrontendClient::IsMetadataSend() const {
  return is_metadata_sent_;
}
...
void FrontendProxy::Accept() {
  ...
  try {
    ...
    // TODO change ip here
    tcp::acceptor acceptor{ioc, tcp::endpoint(tcp::v4(), frontend_listen_port_)};
    for (;;) {
...
void FrontendProxy::SendMetadata(const boost::shared_ptr<FrontendClient>& client) {
    boost::beast::multi_buffer buffer;
    update_metadata_lock_.lock();
