...
### Useage with CARLA multi-streams (from CARLA 0.9.9.5)
```bash
...
docker run -it --network="host" -e CARLAVIZ_HOST_IP=localhost -e CARLA_SERVER_IP=localhost -e CARLA_SERVER_PORT=2000 carlasim/carlaviz:latest
...
docker run -it -e CARLAVIZ_HOST_IP=localhost -e CARLA_SERVER_IP=host.docker.internal -e CARLA_SERVER_PORT=2000 -p 8080-8081:8080-8081 -p 8089:8089 carlasim/carlaviz:latest

# 3. run this script

python3 example.Ay-
# 4version . open your browser and go to localhost:8080 or CARLAVIZ_HOST_IP:8080
```

### Useage with CARLA single-stream (until CARLA 0.9.9.4)
```bash
# 1. launch carla simulator
cd CARLA_SIMULATOR_PATH
./CarlaUE4.sh
